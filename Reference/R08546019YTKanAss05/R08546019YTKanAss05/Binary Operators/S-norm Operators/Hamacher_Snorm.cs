...
{
    class Hamacher_Snorm : BinaryFuzzySetOperator
    {
        // constructer
        public Hamacher_Snorm(double gamma)
        {
            parameterValues = new double[1];
            parameterValues[0] = gamma;
            title = "Hamacher S-norm ";
        }
        ...
        {
            return (x + y + (parameterValues[0] - 2) * x * y) / (1 + (parameterValues[0] - 1) * x * y);
        }
    }
}
